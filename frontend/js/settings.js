...
const SettingsController = {
  ...
  loadSettings() {
    // API URL
    const apiUrl = localStorage.getItem(APP_CONFIG.STORAGE_KEYS.API_URL) || APP_CONFIG.API_BASE_URL;
    const urlInput = document.getElementById('settingApiUrl');
    if (urlInput) urlInput.value = apiUrl;

    // Thresholds
    const savedThresholds = JSON.parse(localStorage.getItem(APP_CONFIG.STORAGE_KEYS.THRESHOLDS)) || APP_CONFIG.DEFAULT_THRESHOLDS;
    ...
    if (medInput) medInput.value = savedThresholds.medium || 0.40;
  ...
  updateThresholdDisplay() {
    ...
    if (critDisplay) critDisplay.textContent = formatPercent(parseFloat(critVal) || 0.85);
    if (highDisplay) highDisplay.textContent = formatPercent(parseFloat(highVal) || 0.70);
    if (medDisplay) medDisplay.textContent = formatPercent(parseFloat(medVal) || 0.40);
  },
  ...
  async testConnection() {
    ...
    if (statusText) statusText.innerHTML = `<span style="color: var(--text-secondary); font-family: var(--font-mono);"><i class="bi bi-arrow-repeat"></i> Testing connectivity...</span>`;
    ...
    try {
      ...
      if (res.ok) {
        ...
        Toast.success('API Connected', `FastAPI backend is responsive (${latency}ms).`);
      } else {
      ...
      Toast.warning('Backend Offline', 'Could not reach server at that address. Demo live stream is active.');
    }
  ...
  saveSettings() {
    ...
    const critical = parseFloat(document.getElementById('threshCritical').value);
    const high = parseFloat(document.getElementById('threshHigh').value);
    const medium = parseFloat(document.getElementById('threshMed')?.value || 0.40);

    localStorage.setItem(APP_CONFIG.STORAGE_KEYS.API_URL, url);
    localStorage.setItem(APP_CONFIG.STORAGE_KEYS.THRESHOLDS, JSON.stringify({
      critical,
      high,
      medium,
      low: 0.20,
    ...
    Toast.success('Settings Saved', 'Risk thresholds and connection configurations updated.');
  },
